...
(function(grammar) {
	...
	//TODO parse it and generate the parse table in a better way...
	function buildProdTable (text) {

		var prodTable = {}, array;
		var nonTermPattern = /.*(::=|->)\n/g;

		array = text.match(nonTermPattern);
		for (var j = 0; j < array.length; j++) {
			var rightSidePattern = array[j] + "([\\s\\S]*?)\\n\\t*\\n";
			var re = new RegExp(rightSidePattern, "g");

			var rightSide = text.match(re);
			if (rightSide !== null) {
				rightSide[0] = rightSide[0].replace(nonTermPattern, "");
				rightSide[0] = rightSide[0].replace(/\t*/g, "");
				rightSide[0] = rightSide[0].split("\n");

				while (rightSide[0].indexOf('') >= 0) {
					var index = rightSide[0].indexOf('');
					rightSide[0].splice(index, 1);
				}

				for (var i = 0 ; i < rightSide[0].length; i++) {
					var el = rightSide[0][i].split(' ');
					rightSide[0][i] = el;
				}

				array[j] = array[j].replace(/ *::=\n*/g,"");
				prodTable[array[j]] = rightSide[0];
			}
		}

		for (var prop in prodTable) {
			if (prodTable.hasOwnProperty(prop)) {
				for (j = 0; j < prodTable[prop].length ; j++) {
					var productions =  prodTable[prop][j];
					for (var k = 0; k < productions.length; k++) {
						var prod = productions[k];
						if (isSymbolAction(prod) && !prodTable[prod] ) {
							prodTable[prod] = [['$']];
						}
					}
				}
			...
			if (prodTable.hasOwnProperty(nonTerminal)){
				...
				for (var i = 0; i < productions.length; i++) {
					...
					if (first.indexOf("$") > -1) {
						...
						if (follow.indexOf('$') > -1) {
							console.log("UEFUIEFIUENFUIEUIFNEIUFNEIUFN");
	...
	//console.log(grammar.isLL1());

//	grammar.printParseTable = function printParseTable (parseTable) {
//		for (var nonTerminal in prodTable) {
//			for (var terminal in parseTable[nonTerminal]) {
//				console.log("M["+nonTerminal+", "+terminal+"] = " + parseTable[nonTerminal][terminal] );
//			}
//		}
//	};
